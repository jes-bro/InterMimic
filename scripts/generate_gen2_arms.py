...
def main(argv=None):
    ...
    written = []
    for name, arch, refs, recipe, fold in cells():
        ...
        slurm = SLURM_TMPL.format(name=name, arch=arch, refs=refs, recipe=recipe,
                                  fold=fold, test=" ".join(FOLDS[fold]),
                                  ret_guard=ret_guard)
        for path, content in [
...
# (test bodies: {test}). Generated by scripts/generate_gen2_arms.py.
...
echo "[teacher] GEN2 {name}: arch={arch} refs={refs} recipe={recipe} fold=f{fold} num_envs=$NUM_ENVS"
