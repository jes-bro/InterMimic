...
#SBATCH --job-name="tch-g3_omomo_geoall_src1_realonly__f0"
#SBATCH --output=teacher-g3_omomo_geoall_src1_realonly__f0-%j.out
...
# Eval when done:  HELDOUT="sub10 sub13 sub16" sh scripts/eval_one.sh g3_omomo_geoall_src1_realonly__f0
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_g3_omomo_geoall_src1_realonly__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_g3_omomo_geoall_src1_realonly__f0.yaml
...
if ! grep -qE '^\s*raggedMotionData:\s*[Tt]rue' "$CFG_ENV"; then
    echo "[teacher] ERROR: g3_omomo_geoall_src1_realonly__f0 without raggedMotionData in $CFG_ENV" >&2; exit 1
fi
...
echo "[teacher] G3 RECIPE g3_omomo_geoall_src1_realonly__f0 (OMOMO data): 43 bodies, no betas, gate resets=true, rollout 50, buf=12.0 num_envs=$NUM_ENVS"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_g3_omomo_geoall_src1_realonly__f0/nn/"
echo "[teacher] ABLATION realonly of g3_omomo_geoall_src1__f0: subjectBodies 43 -> the 13 real OMOMO bodies (no synthetic sub1xx bodies)"
