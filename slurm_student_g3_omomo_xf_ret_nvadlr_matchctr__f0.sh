...
#SBATCH --job-name="stu-g3_omomo_xf_ret_nvadlr_matchctr__f0"
#SBATCH --output=student-g3_omomo_xf_ret_nvadlr_matchctr__f0-%j.out
...
# 3356 clips). NO body wire. Differs from the plain launcher only in names + cfgs.
# Early check: "[distill] matched pairs this rollout" should be in the thousands.
# On GCP / any machine without Slurm: NUM_ENVS=1024 sh scripts/gcp_run_in_tmux.sh $0 omomo_xf_matchctr
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_omomo_xf_ret_nvadlr_matchctr__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_omomo_xf_ret_nvadlr_matchctr__f0.yaml
...
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_omomo_xf_ret_nvadlr_matchctr__f0/nn/"
