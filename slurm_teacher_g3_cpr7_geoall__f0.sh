...
#SBATCH --job-name="tch-g3_cpr7_geoall__f0"
#SBATCH --output=teacher-g3_cpr7_geoall__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# g3 RECIPE ON THE EGOEXO4D CPR RECONSTRUCTION, THE 7 PEOPLE WITH THE MOST
# CLIPS: 17 clips from sub517 (4) 509 (3) 512 564 567 569 579 (2 each) x f0's 43 bodies,
# every clip with its own reconstructed manikin (3.7 kg, STATIC in the
# reference). Hand-written from slurm_teacher_g3_bball7_geoall__f0.sh.
# 7-day walltime; the auto-resume below is the fallback if the node dies.
# Eval when done:  HELDOUT="sub10 sub13 sub16" sh scripts/eval_one.sh g3_cpr7_geoall__f0
#
# --mem=64G: 731 (body, clip) motions padded to the 691-frame longest clip is
# ~4.4 GB of motion. SAME converted set and SAME merged tree as the cpr13 arm
# (only dataSub differs); the full 13-subject retarget + merge serves both.
#
# BEFORE THE FIRST SUBMISSION, in this order (each refuses to run on missing input):
#   1. rclone ~/cari4d_cpr (bundles + manifest) + scripts/cpr_subject_betas.npz
#   2. MANIFEST=... BUNDLES_ROOT=... sbatch scripts/slurm_cari4d_cpr_convert.sh
#   3. the KNEE PASS by hand, conda env active (printed at the end of step 2):
#      relabel_contact_cpr.py --src-dir InterAct/behave_cari4d_cpr --dst-dir InterAct/behave_cari4d_cpr_kn ...
#      -- and NO hand/foot relabel: the sphere-model scripts are wrong for a manikin
#   4. sbatch --array=0-12 --exclude=simurgh6,simurgh2 scripts/slurm_cari4d_cpr_retarget.sh
#   5. the merge_retarget_trees.py command in that script's header
# Expect InterAct/behave_cari4d_cpr_kn with 23 clips and
# InterAct/behave_cari4d_cpr_f0_bodymajor with 46 bodies (43 training + the
# 3 held-out eval bodies) x 23 = 1058 files.
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_g3_cpr7_geoall__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_g3_cpr7_geoall__f0.yaml
...
if ! grep -qE '^\s*objectMass:' "$CFG_ENV"; then
    echo "[teacher] ERROR: cpr arm without objectMass in $CFG_ENV" >&2; exit 1
fi
...
for s in sub509 sub512 sub517 sub564 sub567 sub569 sub579; do
    if ! ls "$MF"/${s}_*.pt >/dev/null 2>&1; then
        echo "[teacher] ERROR: $MF has no ${s}_* clips -- run scripts/slurm_cari4d_cpr_convert.sh + the knee pass" >&2; exit 1
    fi
    if ! ls "$RT"/sub1/${s}_*.pt >/dev/null 2>&1; then
        echo "[teacher] ERROR: $RT has no ${s}_* clips under body sub1 -- run the retarget" \
             "array + merge in scripts/slurm_cari4d_cpr_retarget.sh" >&2; exit 1
    fi
done
...
echo "[teacher] G3 RECIPE g3_cpr7_geoall__f0 (EgoExo4D CPR, top-7 people, 17 clips, static 3.7 kg manikin): 43 bodies, no betas, gate resets=true, rollout 50, buf=12.0 num_envs=$NUM_ENVS"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_g3_cpr7_geoall__f0/nn/"
