...
#SBATCH --job-name="stu-g3_src1_xf_ret_nvadlr__f0"
#SBATCH --output=student-g3_src1_xf_ret_nvadlr__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, SRC1, XF / RET / NVADLR: the single sub1 g3 teacher (the ablation base source) -- a one-source student, 43 target bodies
# distilled into one 6-token transformer (9594), normval + adaptive LR (KL 0.06) student (InterMimicDistillG3 via
# intermimic.run_distill). Data = the base teacher arm's verbatim (see the env
# cfg header). 7-day walltime; auto-resume below.
# On GCP (no Slurm): sh scripts/gcp_run_in_tmux.sh $0 src1_xf   -- shape: a2-highgpu-4g (a2-highgpu-2g if the srcall13 RSS shows the ragged factor near 1.0)
#
# BEFORE THE FIRST SUBMISSION, on the machine that holds the checkpoints + data:
#   python3 scripts/collect_g3_teachers.py --omomo-sources 1 --out checkpoints/teachers/g3_src1
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_src1_xf_ret_nvadlr__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_src1_xf_ret_nvadlr__f0.yaml
...
echo "[student] SRC1 XF / RET / NVADLR student: the single sub1 g3 teacher (the ablation base source) -- a one-source student, 43 target bodies -> 6-token transformer (9594), normval + adaptive LR (KL 0.06), num_envs=$NUM_ENVS"
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_src1_xf_ret_nvadlr__f0/nn/"
