...
#SBATCH --job-name="tch-src6_adlr"
#SBATCH --output=teacher-src6_xf_aug_adlr-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# ADAPTIVE-LR arm (branch source-teacher-staged): identical to the no-stage
# src6_xf_aug teacher EXCEPT lr_schedule=adaptive (KL-target 0.008, start 2e-4).
...
# Saves to checkpoints/smplx_teacher_src6_xf_aug_adlr/nn/.
...
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_src6_xf_aug_adlr.yaml
...
echo "[teacher] src6 adaptive-LR  host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/${EXP}/nn/"
