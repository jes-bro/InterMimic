...
#SBATCH --job-name="stu-g3_act_xf_ret_nvadlr_matchctr_body__f0"
#SBATCH --output=student-g3_act_xf_ret_nvadlr_matchctr_body__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, ACTIVITIES, XF / RET / NVADLR + MATCHCTR: the plain act XF student plus the
...
# (cohortClips 4; a no-op on the activity data, one clip per object) AND the body wire
# (studentBodyFeatures / body_dim 156) -- matchctr_body = bodyonly + the matched term.
# Differs from the plain launcher only in names + cfgs. Early check: the log's
# "[distill] matched pairs this rollout" line should be in the thousands.
# On GCP / any machine without Slurm: NUM_ENVS=1024 sh scripts/gcp_run_in_tmux.sh $0 act_xf_matchctr_body
# bball7 + soccer15 + cpr13 g3 teachers
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_act_xf_ret_nvadlr_matchctr_body__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_act_xf_ret_nvadlr_matchctr_body__f0.yaml
...
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_act_xf_ret_nvadlr_matchctr_body__f0/nn/"
