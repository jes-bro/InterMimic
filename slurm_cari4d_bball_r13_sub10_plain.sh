...
#SBATCH --job-name="bball-r13_sub10_plain"
#SBATCH --output=cari4d-bball-r13_sub10_plain-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# BBALL_R13_SUB10_PLAIN -- CROSS-BODY, NO RETARGETING. r8_horiz's recipe with one
# knob: robotType. sub10's body is asked to track a reference shaped like the
...
# is supposed to fix. sub10 is 1.481 m at rest -- the SHORTEST real subject; the bball subject is a different
...
#
# MEASURED: retargeting sub100 -> sub10 cut contact error 8.62 -> 0.99 cm,
# against 4.77 -> 0.68 for sub2. The un-retargeted mismatch is nearly DOUBLE
# here, so if retargeting matters on this motion, this is the pair that shows
# it -- sub2's 4.77 cm may simply be small enough for the policy to absorb.
#
...
#
# READ IT AGAINST r14_sub10_ret at the same sim step. Those two differ in the
# reference alone, so a difference between them IS the retargeting.
#     grep -h -A 4 "by ref-contact" cari4d-bball-r13_sub10_plain-*.out | tail -8
#     grep -h -A 6 "TERMINATION REASONS" cari4d-bball-r13_sub10_plain-*.out | tail -16
#
#
source ~/.bashrc
conda deactivate
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_cari4d_bball_r13_sub10_plain_train.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_cari4d_bball_r13_sub10_plain_train.yaml
...
if ! grep -qE '^\s*motion_file:\s*InterAct/behave_cari4d_optj3d_cf2\s*$' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: motion_file is not behave_cari4d_optj3d_cf2 in $CFG_ENV -- the relabelled contact data IS the experiment (plain _cf would duplicate r5)" >&2; exit 1
fi
...
if [ ! -d "$MOTION_DIR" ]; then
    echo "[bball-r13_sub10_plain] ERROR: $MOTION_DIR not found. Build it first:" >&2
...
if ! grep -qE '^\s*rewardShape:\s*geometric\b' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: rewardShape is not 'geometric' in $CFG_ENV -- the reward shape IS this experiment (absent = a duplicate of r6_cf2)" >&2; exit 1
fi
...
if ! grep -qE '^\s*robotType:\s*"smplx/smplx_omomo_sub10\.xml"' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: robotType is not smplx_omomo_sub10.xml in $CFG_ENV -- the cross-body swap IS this experiment" >&2; exit 1
fi
# Guard: the reference must stay the ORIGINAL. Pointed at the retargeted dir this
# would be r12, not its baseline.
if ! grep -qE '^\s*motion_file:\s*InterAct/behave_cari4d_optj3d_cf2\s*$' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: motion_file is not the unretargeted behave_cari4d_optj3d_cf2 -- this arm is the NO-retargeting baseline" >&2; exit 1
fi
...
if ! grep -qE '^\s*obsHorizons:\s*\[' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: no obsHorizons in $CFG_ENV -- the horizon set IS this experiment (absent = a duplicate of r7_geom)" >&2; exit 1
fi
...
if [ "$NOBS" != "$((NH * 1599))" ]; then
    echo "[bball-r13_sub10_plain] ERROR: numObs=$NOBS but $NH horizons x 1599 = $((NH * 1599)) in $CFG_ENV" >&2; exit 1
fi
...
if ! grep -qE '^\s*rolloutLength:\s*50\b' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: rolloutLength not 50 in $CFG_ENV -- r6 must match r5 here or the relabel is confounded with a coverage change" >&2; exit 1
fi
...
if ! grep -qE '^\s*stateInit:\s*"Hybrid"' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: stateInit not Hybrid in $CFG_ENV -- short rollout without Hybrid only truncates episodes" >&2; exit 1
fi
...
if grep -qE '^\s*physicalBufferSize:' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: physicalBufferSize present in $CFG_ENV -- rolloutLength 50 un-gates PSI; that is a second variable" >&2; exit 1
fi
# Guards inherited from r2_warm: this arm keeps that termination regime exactly.
if ! grep -qE '^\s*resetThresholds:' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: resetThresholds block missing from $CFG_ENV" >&2; exit 1
fi
if ! grep -qE '^\s*human:\s*0\.5' "$CFG_ENV"; then
    echo "[bball-r13_sub10_plain] ERROR: human reset not set to 0.5 in $CFG_ENV -- it is what keeps the crawl exploit dead" >&2; exit 1
fi
for KNOB in object igRatio contactSteps; do
    if ! grep -qE "^\s*${KNOB}:\s*[Ff]alse" "$CFG_ENV"; then
        echo "[bball-r13_sub10_plain] ERROR: resetThresholds.${KNOB} not false in $CFG_ENV -- object-side resets must stay off" >&2; exit 1
    fi
done

echo "[bball-r13_sub10_plain] invocation: python -u -m intermimic.run --task InterMimic --cfg_env $CFG_ENV --cfg_train $CFG_TRAIN --headless --output checkpoints  (slurm=$0 job=$SLURM_JOB_ID)"
echo "[bball-r13_sub10_plain] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_cari4d_bball_r13_sub10_plain/nn/"
...
RESUME_FROM=""
if [ -f "$CKPT" ]; then
    RESUME_FROM="$CKPT"; echo "[bball-r13_sub10_plain] RESUMING own run from ${CKPT}"
else
    echo "[bball-r13_sub10_plain] first launch: FRESH START (no warm start possible -- numObs 9594 vs the sub2 teacher's 3198)"
fi
