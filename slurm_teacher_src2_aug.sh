...
#SBATCH --job-name="tch-src2_aug"
#SBATCH --output=teacher-src2_aug-%j.out
...
#
# Runs from repo root. Saves to checkpoints/smplx_teacher_src2_aug/nn/.
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_src2_aug.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_src2_aug.yaml

echo "[teacher] source=sub2 x 13 real + 40 synthetic bodies, neutral betas + body-norm + pose, no staging"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_src2_aug/nn/"
