...
#SBATCH --job-name="stu-g3_act_halves_xf_ret_nvadlr__f0"
#SBATCH --output=student-g3_act_halves_xf_ret_nvadlr__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, ACTIVITIES, XF / RET / NVADLR: activity HALVES teachers bball7 + soccer7 + soccer8 + cpr7 + cpr6
# (5 checkpoints, 35 sources) distilled into one 6-TOKEN TRANSFORMER student
...
# On GCP (no Slurm): sh scripts/gcp_run_in_tmux.sh $0 act_halves_xf   -- fits a2-highgpu-1g
#
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_act_halves_xf_ret_nvadlr__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_act_halves_xf_ret_nvadlr__f0.yaml
...
echo "[student] ACTIVITY XF/RET/NVADLR student: bball7+soccer7+soccer8+cpr7+cpr6 -> 6-token transformer (9594), normval + adaptive LR (KL 0.06), per-object physics, RAGGED, num_envs=$NUM_ENVS"
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_act_halves_xf_ret_nvadlr__f0/nn/"
