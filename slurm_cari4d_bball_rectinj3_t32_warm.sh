...
#SBATCH --job-name="bball-rectinj3_t32_warm"
#SBATCH --output=cari4d-bball-rectinj3_t32_warm-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# RECTINJ3_T32_WARM bball experiment: identical to smplx_cari4d_bball_overfit EXCEPT the
...
# (checkpoints/smplx_cari4d_bball_rectinj3_t32_warm/nn/) -- writes NOTHING into the
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_cari4d_bball_rectinj3_t32_warm_train.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_cari4d_bball_rectinj3_t32_warm_train.yaml

# Guard: this experiment IS the resets-off arm -- refuse to run without the block.
if ! grep -qE '^\s*resetThresholds:' "$CFG_ENV"; then
    echo "[bball-rectinj3_t32_warm] ERROR: resetThresholds block missing from $CFG_ENV" >&2; exit 1
fi
if ! grep -qE '^\s*human:\s*[Ff]alse' "$CFG_ENV"; then
    echo "[bball-rectinj3_t32_warm] ERROR: human reset not disabled in $CFG_ENV -- this IS the rectinj3_t32_warm knob" >&2; exit 1
fi

echo "[bball-rectinj3_t32_warm] invocation: python -u -m intermimic.run --task InterMimic --cfg_env $CFG_ENV --cfg_train $CFG_TRAIN --headless --output checkpoints  (slurm=$0 job=$SLURM_JOB_ID)"
echo "[bball-rectinj3_t32_warm] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_cari4d_bball_rectinj3_t32_warm/nn/"
...
RESUME_FROM=""
if [ -f "$CKPT" ]; then
    RESUME_FROM="$CKPT"; echo "[bball-rectinj3_t32_warm] RESUMING own run from ${CKPT}"
else
    echo "[bball-rectinj3_t32_warm] first launch: EXPLICIT warm start from smplx_teachers_new/sub2.pth (per cfg; Jess-approved)"
fi
