...
#SBATCH --job-name="stu-g3_omomo_all13t_mlp_ret_stock__f0"
#SBATCH --output=student-g3_omomo_all13t_mlp_ret_stock__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, OMOMO, MLP / RET / STOCK: the single srcall13 teacher (one checkpoint, all 13 sources) -- distillation as pure architecture/optimizer transfer distilled
# into one MLP student (InterMimicDistillG3 via intermimic.run_distill). Data =
# the srcall13 teacher's verbatim (13 sources x f0's 43 bodies, merged contact-
# retarget tree, RAGGED, cpuMotionData). Hand-written from
# slurm_teacher_g3_omomo_geoall_srcall13__f0.sh. 7-day walltime; the auto-resume
# below is the fallback if the node dies.
# On GCP (no Slurm): sh scripts/gcp_run_in_tmux.sh $0 omomo_all13t_mlp
#
...
#          --omomo-arms srcall13 --out checkpoints/teachers/g3_omomo_all13t
#      (refuses a partial set; writes teachers.yaml with each teacher's epoch)
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_omomo_all13t_mlp_ret_stock__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_omomo_all13t_mlp_ret_stock__f0.yaml
...
echo "[student] OMOMO MLP/RET/STOCK student: 1 all-source teacher -> MLP 1024/1024/512, 6 student horizons (9594), constant LR, RAGGED, num_envs=$NUM_ENVS"
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_omomo_all13t_mlp_ret_stock__f0/nn/"
