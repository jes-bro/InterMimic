...
#SBATCH --job-name="stu-g3_omomo_xf_ret_nvadlr_matchctr_body__f0"
#SBATCH --output=student-g3_omomo_xf_ret_nvadlr_matchctr_body__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, OMOMO, XF / RET / NVADLR + MATCHCTR: the plain omomo XF student plus the
# matched-frame contrastive term (same clip / same frame / other body positives found in
# each rollout; InfoNCE on a trunk projection, coef 0.1) with cohort clip sampling
# (cohortClips 4: envs on one object follow a leader's clip so pairs exist on OMOMO's
# 3356 clips) AND the body wire (studentBodyFeatures / body_dim 156) -- matchctr_body =
# bodyonly + the matched term. Differs from the plain launcher only in names + cfgs.
# Early check: "[distill] matched pairs this rollout" should be in the thousands.
# On GCP / any machine without Slurm: NUM_ENVS=1024 sh scripts/gcp_run_in_tmux.sh $0 omomo_xf_matchctr_body
# The 13 per-source g3 OMOMO teachers distilled
...
# On GCP (no Slurm): sh scripts/gcp_run_in_tmux.sh $0 omomo_xf
#
...
#          --omomo-sources 1 2 3 5 6 7 8 9 11 12 14 15 17 --out checkpoints/teachers/g3_omomo
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_omomo_xf_ret_nvadlr_matchctr_body__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_omomo_xf_ret_nvadlr_matchctr_body__f0.yaml
...
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_omomo_xf_ret_nvadlr_matchctr_body__f0/nn/"
