...
#SBATCH --job-name="stu-g3_bball7_xf_ret_nvadlr__f0"
#SBATCH --output=student-g3_bball7_xf_ret_nvadlr__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, BBALL7, XF / RET / NVADLR: the bball7 g3 teacher alone (7 EgoExo4D people, 48 clips, per-clip balls) -- the basketball-only student
...
# On GCP (no Slurm): sh scripts/gcp_run_in_tmux.sh $0 bball7_xf   -- shape: a2-highgpu-1g
#
# BEFORE THE FIRST SUBMISSION, on the machine that holds the checkpoints + data:
#   python3 scripts/collect_g3_teachers.py --activities bball7 --out checkpoints/teachers/g3_bball7
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_bball7_xf_ret_nvadlr__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_bball7_xf_ret_nvadlr__f0.yaml
...
echo "[student] BBALL7 XF / RET / NVADLR student: the bball7 g3 teacher alone (7 EgoExo4D people, 48 clips, per-clip balls) -- the basketball-only student -> 6-token transformer (9594), normval + adaptive LR (KL 0.06), num_envs=$NUM_ENVS"
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_bball7_xf_ret_nvadlr__f0/nn/"
