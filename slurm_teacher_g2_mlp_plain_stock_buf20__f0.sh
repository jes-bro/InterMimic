...
#SBATCH --job-name="tch-g2_mlp_plain_stock_buf20__f0"
#SBATCH --output=teacher-g2_mlp_plain_stock_buf20__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# GEN-2 grid cell: arch=mlp refs=plain recipe=stock fold=f0
# buffer_mult=20 (test bodies: sub10 sub13 sub16). Generated by scripts/generate_gen2_arms.py.
# 24h walltime for fast iteration -- resubmit to auto-resume.
# Eval when done:  HELDOUT="sub10 sub13 sub16" sh scripts/eval_one.sh g2_mlp_plain_stock_buf20__f0
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_g2_mlp_plain_stock_buf20__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_g2_mlp_plain_stock_buf20__f0.yaml
...
# Buffer guard: the cfg must carry exactly this cell's multiplier (20.0).
if ! grep -qE '^\s*default_buffer_size_multiplier:\s*20\.0' "$CFG_ENV"; then
    echo "[teacher] ERROR: buffer multiplier in $CFG_ENV is not 20.0" >&2; exit 1
fi
...
echo "[teacher] GEN2 g2_mlp_plain_stock_buf20__f0: arch=mlp refs=plain recipe=stock fold=f0 buf=20.0 num_envs=$NUM_ENVS"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_g2_mlp_plain_stock_buf20__f0/nn/"
