...
#SBATCH --job-name="tch-g3_omomo_geoall_srchalf6__f0"
#SBATCH --output=teacher-g3_omomo_geoall_srchalf6__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# HALF-A g3 teacher: 6 sources (sub1 sub3 sub5 sub12 sub15 sub17, 1677 clips)
# x 43 bodies, RAGGED motion storage. Its twin srchalf7 holds the other 7
...
# Eval when done:  HELDOUT="sub10 sub13 sub16" sh scripts/eval_one.sh g3_omomo_geoall_srchalf6__f0
#
# --mem=320G is ~30% over the RAGGED budget under the PADDED loader's 2.02x model
# (14.2 + 2.02 x 113 GiB = 242; scripts/motion_memory_budget.py). Padded would
# have needed ~655 GiB. If the srcall13 run has reported MaxRSS by the time
# this is submitted, size from its measured factor instead.
#
# BEFORE THE FIRST SUBMISSION build the merged retarget tree (symlinks):
#   python3 scripts/merge_retarget_trees.py \
#       --sources InterAct/OMOMO_retarget_contact_src1 InterAct/OMOMO_retarget_contact_src3 \
#                 InterAct/OMOMO_retarget_contact_src5 InterAct/OMOMO_retarget_contact_src12 \
#                 InterAct/OMOMO_retarget_contact_src15 InterAct/OMOMO_retarget_contact_src17 \
#       --out InterAct/OMOMO_retarget_contact_srchalf6 \
#       --bodies-from isaacgym/src/intermimic/data/cfg/omomo_teacher_g3_omomo_geoall_srchalf6__f0.yaml
# Expect: linked 72111 clips across 43 bodies.
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_g3_omomo_geoall_srchalf6__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_g3_omomo_geoall_srchalf6__f0.yaml
...
if ! grep -qE '^\s*raggedMotionData:\s*[Tt]rue' "$CFG_ENV"; then
    echo "[teacher] ERROR: srchalf6 without raggedMotionData in $CFG_ENV" >&2; exit 1
fi
...
for s in sub1 sub3 sub5 sub12 sub15 sub17; do
...
echo "[teacher] G3 RECIPE g3_omomo_geoall_srchalf6__f0 (OMOMO data, half A = 6 sources, RAGGED motion): 43 bodies, no betas, gate resets=true, rollout 50, buf=12.0 num_envs=$NUM_ENVS"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_g3_omomo_geoall_srchalf6__f0/nn/"
