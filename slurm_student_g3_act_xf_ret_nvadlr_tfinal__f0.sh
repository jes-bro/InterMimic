...
#SBATCH --job-name="stu-g3_act_xf_ret_nvadlr_tfinal__f0"
#SBATCH --output=student-g3_act_xf_ret_nvadlr_tfinal__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, ACTIVITIES, XF / RET / NVADLR, TFINAL: the activity XF student distilled from
# the FINAL teacher checkpoints (the Sep-16 run used mid-training teachers). Identical to
# slurm_student_g3_act_xf_ret_nvadlr__f0.sh except names, the cfgs, and the teacher dir:
# collect the three teachers' LATEST snapshots into checkpoints/teachers/g3_act_tfinal
# (step 2 below; pull them fresh from the bucket first). The collect step PRINTS the epoch
# it picked per teacher -- record those; cpr13 may still be training, and "final" means
# "latest at launch".
...
# On GCP (no Slurm): NUM_ENVS=1024 sh scripts/gcp_run_in_tmux.sh $0 act_xf_tfinal   -- fits a2-highgpu-1g
#
...
#          --out checkpoints/teachers/g3_act_tfinal
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_act_xf_ret_nvadlr_tfinal__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_act_xf_ret_nvadlr_tfinal__f0.yaml
...
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_act_xf_ret_nvadlr_tfinal__f0/nn/"
