...
#SBATCH --job-name="tch-src2_mlp_nvadlr"
#SBATCH --output=teacher-src2_mlp_normval_adlr-%j.out
...
#
# Runs from repo root. Saves to checkpoints/smplx_teacher_src2_mlp_normval_adlr/nn/.
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_src2_mlp_lowbuf.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_src2_mlp_normval_adlr.yaml
...
echo "[teacher] MLP + NORMVAL+ADLR (fresh from epoch 0): source=sub2 x 13 real + 40 synthetic bodies, neutral betas + body-norm + pose, normalize_value=True, lr_schedule=adaptive"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_src2_mlp_normval_adlr/nn/"
