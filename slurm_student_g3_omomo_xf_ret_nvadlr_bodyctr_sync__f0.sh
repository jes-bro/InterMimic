...
#SBATCH --job-name="stu-g3_omomo_xf_ret_nvadlr_bodyctr_sync__f0"
#SBATCH --output=student-g3_omomo_xf_ret_nvadlr_bodyctr_sync__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, OMOMO, XF / RET / NVADLR + BODYCTR_SYNC (ARM A + twinCoReset): bodyctr with
# twin pairs that reset as a unit, so the contrastive term sees synced pairs for whole
# episodes (act bodyctr measured ~5% valid pairs per rollout; expect >90% here -- check
# the "[distill] twin pairs valid this rollout" line early). Differs from the bodyctr
# launcher only in names + cfgs. The 13 per-source g3 OMOMO teachers distilled
...
# On GCP (no Slurm): sh scripts/gcp_run_in_tmux.sh $0 omomo_xf_bodyctr_sync
#
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_omomo_xf_ret_nvadlr_bodyctr_sync__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_omomo_xf_ret_nvadlr_bodyctr_sync__f0.yaml
...
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_omomo_xf_ret_nvadlr_bodyctr_sync__f0/nn/"
