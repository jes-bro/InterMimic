...
#SBATCH --job-name="tch-src6_xf"
#SBATCH --output=teacher-src6_xf-%j.out
...
#
# Runs from repo root. Saves to checkpoints/smplx_teacher_src6_xf/nn/.
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_src6_xf.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_src6_xf.yaml
...
echo "[teacher] source=sub6 x 13 train bodies (sub13 held out), TRANSFORMER + neutral betas + body-norm + pose, no staging"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_src6_xf/nn/"
