...
#SBATCH --job-name="stu-g3_omomo_xf_ret_nvadlr_tfinal_nopose__f0"
#SBATCH --output=student-g3_omomo_xf_ret_nvadlr_tfinal_nopose__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# METHOD CANDIDATE nopose OF student_g3_omomo_xf_ret_nvadlr_tfinal__f0: the OMOMO
# XF student on the FINAL teachers with rewardTerms.pose.enable false (no relative
# joint-angle reward factor). NOT an ablation: the teacher nopose arm beat the
# with-pose teacher, so this tests dropping the term from the method itself.
# Identical to slurm_student_g3_omomo_xf_ret_nvadlr_tfinal__f0.sh
# except names, the cfgs and the METHOD CANDIDATE echo; SAME teacher dir
# (checkpoints/teachers/g3_omomo_tfinal -- already collected for the base, and the
# teachers keep their own pose term). 7-day walltime; the auto-resume below is the
# fallback if the node dies. Compare at a MATCHED EPOCH against the base, past the
# epoch-2900 PPO warm-in (see the env cfg header), never at matched wall time.
# On GCP / any machine without Slurm:
#   NUM_ENVS=1024 sh scripts/gcp_run_in_tmux.sh $0 omomo_xf_tfinal_nopose
#
...
#          --omomo-sources 1 2 3 5 6 7 8 9 11 12 14 15 17 --out checkpoints/teachers/g3_omomo_tfinal
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_omomo_xf_ret_nvadlr_tfinal_nopose__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_omomo_xf_ret_nvadlr_tfinal_nopose__f0.yaml
...
echo "[student] OMOMO XF/RET/NVADLR student: 13 g3 teachers -> 6-token transformer (9594), normval + adaptive LR (KL 0.06), RAGGED, num_envs=$NUM_ENVS"
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_omomo_xf_ret_nvadlr_tfinal_nopose__f0/nn/"
echo "[student] METHOD CANDIDATE nopose of student_g3_omomo_xf_ret_nvadlr_tfinal__f0: rewardTerms.pose.enable true -> false (no relative joint-angle reward factor)"
