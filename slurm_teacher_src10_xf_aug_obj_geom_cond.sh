...
#SBATCH --job-name="tch-src10_xf_aug_obj_geom_cond"
#SBATCH --output=teacher-src10_xf_aug_obj_geom_cond-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# SOURCE-TEACHER + OBJECT AUG run: source=sub10 driving 13 real + 40 synthetic
# bodies AND size/pose- AND SHAPE-perturbed objects (scale/yaw/translate +
# anisotropic geometry variants, drop object terms + hold), body-conditioned (neutral betas), NO staging -- one RL run.
#
...
#
# Runs from repo root. Saves to checkpoints/smplx_teacher_src10_xf_aug_obj_geom_cond/nn/.
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_teacher_src10_xf_aug_obj_geom_cond.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_teacher_src10_xf_aug_obj_geom_cond.yaml
...
echo "[teacher] source=sub10 x 13 real + 40 synthetic bodies, neutral betas + body-norm + pose, no staging"
echo "[teacher] host=$(hostname) job=$SLURM_JOB_ID -> checkpoints/smplx_teacher_src10_xf_aug_obj_geom_cond/nn/"
