...
#SBATCH --job-name="stu-g3_omomo_xf_ret_nvadlr_tfinal__f0"
#SBATCH --output=student-g3_omomo_xf_ret_nvadlr_tfinal__f0-%j.out

#SBATCH --mail-user=jesb@stanford.edu
#SBATCH --mail-type=ALL

# STUDENT, OMOMO, XF / RET / NVADLR, TFINAL: the plain OMOMO XF student distilled
# from the FINAL teacher checkpoints (the Sep-16 run used mid-training teachers).
# Identical to slurm_student_g3_omomo_xf_ret_nvadlr__f0.sh except names, the cfgs,
# and the teacher dir: collect the 13 teachers' LATEST snapshots into
# checkpoints/teachers/g3_omomo_tfinal (step 2 below; pull them fresh from the
# bucket first -- the five GCP teachers must have been pushed after they
# finished). 7-day walltime; the auto-resume below is the fallback if the node dies.
# On GCP / any machine without Slurm:
#   NUM_ENVS=1024 sh scripts/gcp_run_in_tmux.sh $0 omomo_xf_tfinal
#
...
CFG_ENV=isaacgym/src/intermimic/data/cfg/omomo_student_g3_omomo_xf_ret_nvadlr_tfinal__f0.yaml
CFG_TRAIN=isaacgym/src/intermimic/data/cfg/train/rlg/omomo_student_g3_omomo_xf_ret_nvadlr_tfinal__f0.yaml
...
echo "[student] host=$(hostname) job=${SLURM_JOB_ID:-none} -> checkpoints/smplx_student_g3_omomo_xf_ret_nvadlr_tfinal__f0/nn/"
